...
def imageBoundingBox(img, M):
    ...
    h, w = img.shape[:2]
    pts = np.array([[0, 0, 1], [0, h - 1, 1], [w - 1, 0, 1], [w - 1, h - 1, 1]])
    pts_transformed = np.dot(M, pts.T).T
    pts_normalized = pts_transformed[:, :2] / pts_transformed[:, 2:]
    minX = np.floor(pts_normalized[:, 0].min())
    minY = np.floor(pts_normalized[:, 1].min())
    maxX = np.ceil(pts_normalized[:, 0].max())
    maxY = np.ceil(pts_normalized[:, 1].max())
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    for i in range(minX, maxX):
        for j in range(minY, maxY):
            pixel = np.dot(M_inv, np.array([i, j, 1.0]))
            ...
            if 0 <= x < w - 1 and 0 <= y < h - 1:
                ...
                if maxX - blendWidth < x <= maxX:
                    weight = float((maxX - x)/blendWidth) 
                
                acc[j, i, 0] += img[int(y), int(x), 0] * weight
                acc[j, i, 1] += img[int(y), int(x), 1] * weight
                acc[j, i, 2] += img[int(y), int(x), 2] * weight
                acc[j, i, 3] += weight
...
def normalizeBlend(acc):
    ...
    alpha = acc[:, :, 3]
    alpha[alpha == 0] = 1
    normalized_img = acc[:, :, :3] / alpha[:, :, np.newaxis]
