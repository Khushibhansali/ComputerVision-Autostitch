...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    radialDistortion = np.power(xn,2) + np.power(yn,2)
    xt = xn * (1 + (k1 * radialDistortion) +  (k2 * np.power(radialDistortion,2)))
    yt = yn * (1 + (k1 * radialDistortion) +  (k2 * np.power(radialDistortion,2)))
